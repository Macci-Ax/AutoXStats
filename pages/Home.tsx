        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="bg-slate-800 rounded-xl p-6 border border-slate-700 shadow-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {prioritizedChamps.map(champ => {
                const leader = getTopDriver(champ);
                if (!leader) return null;
                return (
                  <div key={champ} className="bg-slate-900/50 p-4 rounded-lg flex items-center gap-4 border border-slate-700/50 hover:border-red-500/50 transition cursor-pointer" onClick={() => onNavigate('drivers')}>
                    <div className="bg-slate-800 h-12 w-12 rounded-full flex items-center justify-center text-xs font-bold text-slate-400 border border-slate-600 overflow-hidden">
                       <img src={leader.avatarUrl} alt={leader.name} className="w-full h-full object-cover"/>
                    </div>
                    <div className="flex-1">
                      <div className="flex justify-between items-center mb-1">
                        <span className="font-black text-red-500 text-sm tracking-wider">{champ}</span>
                        <span className="text-xs bg-slate-700 px-2 py-0.5 rounded text-white">{leader.points} Pkt</span>
                      </div>
